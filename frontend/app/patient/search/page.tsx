...
type SearchType = "reports" | "consultations" | "notifications";
...
export default function PatientSearchPage() {
  ...
  const buildSearchUrl = () => {
    ...
    return `${API_BASE_URL}/search/notifications?${params.toString()}`;
  };
  ...
  const getResultId = (item: SearchResult) => {
    return String(
      item.id || item.report_id || item.consultation_id || item.notification_id || ""
    );
  };
  ...
  const getTitle = (item: SearchResult) => {
    ...
    if (searchType === "consultations") {
      return `Consultation with Dr. ${item.doctor_name || "Doctor"}`;
    }
  ...
  const getDescription = (item: SearchResult) => {
    ...
    if (searchType === "consultations") {
      return item.patient_message || "No consultation message available.";
    }

    return item.message || "No notification message available.";
  };
  ...
  const handleOpenResult = (item: SearchResult) => {
    ...
    if (searchType === "reports") {
      router.push(`/patient/report-history/${id}`);
      return;
    }
              ...
              <div className="flex flex-col gap-6 px-8 py-10 text-white md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-3 max-w-2xl text-base text-blue-50">
                    Search your reports, consultations, and notifications from one place.
                  </p>
                </div>
                  ...
                  {searchType === "consultations" && (
                    <div>
                      ...
                      >
                        ...
                        <option value="pending_doctor_acceptance">
